...
$metatags
        ->title('Fake Bill Chuyển Khoản - Công cụ fake bill chuyển khoản chuẩn dùng để seeding')
        ->description('Dịch vụ fake bill ck Vietcombank,Sacombank,MBBank,BIDV,AgriBank,Techcombank,ACB...')
        ->meta('author', 'FakeBillx.Com')
        ->image('https://fakebillx.com/banner123.png');
                ...
                <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
                    ...
                    <div class="sm:flex sm:justify-between sm:items-center mb-5">
                        ...
                        <div class="mb-4 sm:mb-0">
                            <h1 class="text-2xl md:text-3xl text-slate-800 dark:text-slate-100 font-bold">Fake bill chuyển khoản ✨</h1>
<br/><p style="color:red"><b>Lưu Ý: Số tiền không được để dấu phẩy</b></p>
<br/>
                        </div>
...
if ($isAdmin) {
    $sql = "SELECT * FROM bank1 WHERE (active = '1' AND onlyAdmin = '0' OR onlyAdmin = '1') ORDER BY id ASC";
} else {
    $sql = "SELECT * FROM bank1 WHERE (active = '1' AND onlyAdmin = '0') ORDER BY id ASC";
}

$results = DB::query($sql);
foreach ($results as $result) {
  echo '<a href="fake-bill-'.$result['file_name'].'" class="bg-white dark:bg-slate-800 shadow-lg rounded-xl p-5">
                                        <!-- Start -->
                                        <div class="relative">
                                            <center>
                                                Tạo Bill</br>
...
?>
                     ...
                     <div class="flex flex-col col-span-full sm:col-span-6 xl:col-span-4 bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700 mt-4">
                            <header class="px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center">
                                <h2 class="font-semibold text-slate-800 dark:text-slate-100">Demo bill chuyển khoản</h2>
