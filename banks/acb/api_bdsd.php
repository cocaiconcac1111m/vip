  ...
  'amount' => $tiengoc
       ...
       $theme = 'acb01.png';
        ...
        canchinhgiua($sourceImage, 30, 400, imagecolorallocate($sourceImage, 255,0,0), $_SERVER['DOCUMENT_ROOT'].'/fonts/San Francisco/SanFranciscoText-Semibold.otf', 'Ảnh này chỉ để xem demo');
            ...
            imagettftext($sourceImage, 38, 0, 140, 105, imagecolorallocate($sourceImage, 0,0,0), $fontPath.'/San Francisco/SanFranciscoText-Semibold.otf', $_POST['time_dt']);
            $overlayImagePath = $_SERVER['DOCUMENT_ROOT'].'/banks/techcombank/pin_ios/'.$_POST['pin'].'.png';
            imagettftext($sourceImage, 33, 0, 110, 620, imagecolorallocate($sourceImage, 51,51,51), $fontPath.'/Inter/Inter-Light.ttf', str_replace(' ',' - ',$_POST['time_bill']));
            ...
            $date = DateTime::createFromFormat('H:i d/m/Y', $string);
            
            // Lấy ngày và định dạng lại nếu cần
            $ngay = $date->format('d/m/Y');
            imagettftext($sourceImage, 38, 0, 110, 700, imagecolorallocate($sourceImage, 000), $fontPath.'/Inter/Inter-Regular.ttf', 'Thong bao thay doi so du tai khoan');
            imagettftext($sourceImage, 33, 0, 110, 770, imagecolorallocate($sourceImage, 000), $fontPath.'/Inter/Inter-Regular.ttf', 'ACB: TK '.$_POST['stkgui'].'(VND) '.$dau.' '.number_format($_POST['amount']).' luc '.$_POST['time_dt']."\n".$ngay.' So du '.number_format($_POST['sdc']).'. GD:'."\n".$_POST['noidung']);
            $overlayImagePath = $_SERVER['DOCUMENT_ROOT'].'/banks/techcombank/pin_ios/'.$_POST['pin'].'.png';
