...
$url = 'https://www.coingecko.com/price_charts/37016/vnd/max.json';
        $curl = curl_init($url);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        $response = curl_exec($curl);
        $data = json_decode($response, true);
        $lastIndex = count($data['stats']) - 1;
        $tidoi = $data['stats'][$lastIndex][1];
        curl_close($curl);
...
<div class="grid gap-5 md:grid-cols-1">
    ...
    <div class="grid gap-5 md:grid-cols-3">
        ...
        <div class="">
            ...
            <input value="10" id="amount" class="form-input w-full" type="number" placeholder="Ví dụ: 100000">
        </div>

        <div class="">
            <label class="block text-sm mb-2" for="default">Tỉ đối USDT/VND</label>
            <input value="<?php echo $tidoi; ?>" id="tidoi" class="form-input w-full" type="number" placeholder="<?php echo $tidoi; ?>">
        </div>
        
        <div class="">
            <label class="block text-sm mb-2" for="default">Quy đổi sang VND (Tự động)</label>
            <input value="" id="vnd" class="form-input w-full" type="text" disabled placeholder="Số lượng USDT x Tỉ đối">
        </div>
<!--        --><?php //include $_SERVER['DOCUMENT_ROOT'] . '/select-form.php'; ?>
        <div class="">
            ...
            <select onchange="showPin();" id="theme" class="form-select w-full">
                ...
                <option value="ios4g">IOS/IPhone (4G)</option>
            </select>
            <script>
                function showPin() {
...
// Duyệt qua từng thẻ img trong danh sách
        ...
        <div class="">
            ...
            <input id="time_dt" class="form-input w-full" type="text" value="<?= date('G:i') ?>"
                   placeholder="Ví dụ: <?= date('G:i') ?>">
        </div>
                    ...
                    <div class="bg-white dark:bg-slate-800 rounded shadow-lg overflow-auto max-w-lg w-full max-h-full"
                        ...
                        <div class="px-5 py-3 border-b border-slate-200 dark:border-slate-700">
                            <div class="flex justify-between items-center">
                                ...
                                <button class="text-slate-400 dark:text-slate-500 hover:text-slate-500 dark:hover:text-slate-400"
                        ...
                        <div class="px-5 pt-4 pb-1">
                            <div class="text-sm">
                                <div class="font-medium text-slate-800 dark:text-slate-100 mb-2">Chỉnh phần trăm pin của
                                    bill tại đây
                                </div>
                                <div class="space-y-2">
        ...
        <div>
            ...
            >Tạo bill (<?= number_format($tiengoc) ?>)
            </button>
            <button class="btn bg-rose-500 hover:bg-rose-600 w-full text-white" onclick="taoBill('<?= $default ?>')">Xem
                demo
            </button>
    ...
    <div>
        ...
        <div>
            <div class="flex items-center justify-between">
                <label class="block text-sm font-medium mb-1" for="tooltip">Xem demo bill trước khi tải
                    xuống<br/><small>(Nhấn vào ảnh để tải bill xuống máy)</small></label>
...
<script>
    ...
    const formattedDecimal = decimalPart ? Math.round(parseFloat('0.' + decimalPart) * 100).toString().padStart(2, '0') : '00';
    return `${formattedInteger},${formattedDecimal}`;
}

    //amount
    var amountInput = document.getElementById('amount')
    var vndInput = document.getElementById('vnd')
    var tiDoiInput = document.getElementById('tidoi')
    var tidoi = <?php echo $tidoi; ?>;
    // var tidoi = ;

    vndInput.value = formatMoney(`${parseInt(amountInput.value) * parseInt(tidoi)}`)
    amountInput.addEventListener('input', ()=>{
        vndInput.value = formatMoney(`${parseFloat(amountInput.value) * tidoi}`)
    })
    
    tiDoiInput.addEventListener('input', ()=>{
        tidoi = parseFloat(tiDoiInput.value)
        vndInput.value = formatMoney(`${parseFloat(amountInput.value) * tidoi}`)
    })
    ...
    function taoBill(type) {
        ...
        var inputData = {
            ...
            amount: formatMoney(document.getElementById('amount').value),
